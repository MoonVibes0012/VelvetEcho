...
function ramalanOfflineFunc() {
    const pilihan =
        ramalanOffline[
            Math.floor(Math.random() * ramalanOffline.length)
        ];
...
async function hapusJejak() {
    ...
    try {
        const res = await fetch(API + '/jejak', {
            method: 'DELETE'
        });
...
function kirimChatOffline() {
    ...
    setTimeout(() => {
        const balasan =
            balasanOffline[
                Math.floor(Math.random() * balasanOffline.length)
            ];
...
function ubahWarna() {
    warnaIndex =
        (warnaIndex + 1) % warnaList.length;

    document.body.style.background =
        warnaList[warnaIndex];

    notif('🌈 Warna berganti!');
}

// ============================================================
// FPS CONTROL — TARGET 45 FPS
// ============================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('btnRamalan')?.addEventListener(
        'click',
        ramalanOfflineFunc
    );

    $('btnRamalanOnline')?.addEventListener(
        'click',
        ramalanOnlineFunc
    );

    $('btnWarna')?.addEventListener(
        'click',
        ubahWarna
    );
    ...
    $('btnChatOnline')?.addEventListener(
        'click',
        kirimChatOnline
    );
